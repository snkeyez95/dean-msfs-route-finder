...
```
v5.9.0   New: Performance tab — your MSFS smoothness logs (TLOD/OLOD test flights) now live inside ABRP. Lists every logged flight with aircraft, route, TLOD, P99 and VRAM, and opens the full report (charts + dashboard) embedded in the app, with an open-in-browser fallback. Groundwork for folding the standalone performance logger into ABRP (engine relocated under perf/, flight data stored in your AppData folder)
